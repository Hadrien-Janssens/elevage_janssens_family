...
<template>
    <Link
        :href="
            isKitten(kitten)
                ? route('kitten.show', { id: kitten.id })
                : isLitter(kitten)
                  ? route('litter.show', { id: kitten.id })
                  : route('cats.show', { id: kitten.id })
        "
    >
        <div
            class="group relative mx-auto max-w-md cursor-pointer overflow-hidden rounded-lg shadow-lg transition-transform duration-300 hover:scale-105"
        >
            <!-- Bandeau "Réservé" -->
            ...
            <div v-if="isKitten(kitten)" class="absolute top-2 left-2 z-10" @click.stop.prevent="toggleLike">
                <font-awesome-icon v-if="isLiked" icon="fa-solid fa-heart " class="z-50 text-2xl text-red-500" />
