...
const accept = () => {
    document.cookie = 'analytics_consent=yes; path=/; max-age=31536000;  SameSite=Lax';
    emit('update:showBanner', false);
    window.location.reload();
};

const decline = () => {
    document.cookie = 'analytics_consent=no; path=/; max-age=31536000;  SameSite=Lax';
    emit('update:showBanner', false);
    window.location.reload();
};
</script>
...
<template>
    <div v-if="showBanner" class="bg-primary fixed bottom-0 z-50 w-full p-5 pt-10 text-center font-bold text-white">
